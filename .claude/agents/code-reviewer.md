...
- [ ] **Dead Code 감지 — 선언된 ErrorCode/enum 미사용**: 새로 추가된 enum 값이나 ErrorCode 가 실제로 throw/참조되는 곳이 있는지 확인. 선언만 하고 사용하지 않으면 "구현 완료" 착각을 유발
  - 근거: PR#27 CodeRabbit Major (ErrorCode.java:75-81)
  - 체크: `Grep "TRIPO_INSUFFICIENT_CREDIT\|TRIPO_AUTH_FAILED\|TRIPO_RATE_LIMITED\|TRIPO_SERVER_ERROR"` 으로 throw 지점 존재 확인
- [ ] **validateStatusTransition 공유 시 source 분리**: 같은 target 상태(예: REQUESTED)로 전이하는 메서드가 2개 이상이면, 각 메서드의 허용 source 상태가 겹치지 않는지 확인. 겹치면 의도하지 않은 전이 경로가 열림
  - 근거: PR#27 CodeRabbit Major (Showcase3dModel.java:95-106)
  - 예: resetRequest (FAILED/UNAVAILABLE → REQUESTED) 와 resetForRetry (PREPARING → REQUESTED) 가 같은 validate 를 쓰면 PREPARING 에서 resetRequest 가 통과
