...
- ⚠️ **worker SPOF — restart + cgroup + actuator liveness HTTP probe (PR3 한정 보강)**: `restart: unless-stopped`(JVM die 시 exit code 기반 자동 재시작) + cgroup `mem_limit`(api 800m / worker 730m, §2.5+5%) + **bash `/dev/tcp` 로 `/actuator/health/liveness` HTTP probe 기반 docker healthcheck**(PR3, 아래 ✅ 항목 참조, api/worker 동일 명령·동일 그룹). 단 **자동 restart 트리거는 X (docker compose 한계 — healthcheck 결과로 자동 stop/restart 시키는 옵션 미지원, swarm/k8s 만 지원)** — PR3 의 실효 = 가시성 보강 + CD 폴링 차단(`(healthy)` 기반). **검출 가능 (PR3 후)**: JVM 사망 + 부팅 OOM 무한 재시작 + Spring `AvailabilityChangeEvent` self-down(현 코드베이스에 publish 호출 0건이라 사실상 dead-weight, false negative 비용 0) + 9091 응답 가능. **검출 못함 (잔여)**: (a) Kafka 연결 끊김 / `KafkaListenerContainer.stop()` / Consumer thread deadlock, (b) Redisson watchdog 실패로 락 영구 hold, (c) 스케줄러 풀(`spring.task.scheduling.pool.size=3`) 굶음, (d) JVM 좀비 deadlock(9091 응답은 살아 UP 표시) — 모두 컨테이너 State=running·healthcheck UP 유지. 즉 **컴포넌트 레벨 사고는 PR3 도 검출 못 함** — 진짜 검출은 P1-H Prometheus alert 영역(`kafka_consumer_records_consumed_total` lag / `gearshow.admission.queue.size` / `gearshow.scheduler.tasks.active` 등).
...
- ✅ **PR3 완료 (2026-05-20, refactor/worker-liveness-probe — 위 worker SPOF 항목과 통합)**: `application-prod.yml` 에 `management.endpoint.health.probes.enabled=true` 추가. `docker-compose.prod.yml` api/worker 양쪽에 healthcheck 신설(bash 명시 호출 + `/dev/tcp/localhost/9091` + raw HTTP/1.0 GET `/actuator/health/liveness` + `^{"status":"UP"` top-level grep, Dockerfile 무수정). CD 의 worker 폴링을 `State=running` → docker `(healthy)` 기반으로 격상.
  - **표현 정정 (PR2 실현 노트의 부정확성)**: "restart 트리거 자동화(docker `--health-...` exit on failure 정책과 결합)" 은 docker compose 의 실제 동작과 어긋남 — **docker compose 의 healthcheck 결과로 자동 restart 트리거는 불가능**(compose 한계, swarm/k8s 만 지원). PR3 의 실효 = 가시성 보강 + CD 폴링 차단. 진짜 자동 복구는 willfarrell/autoheal 등 별도 컨테이너 도입 영역(본 PR scope 밖).
  - **검출력 정직한 후퇴 (코드 리뷰 반영, 2026-05-20)**: 초기 안은 `application-worker.yml` 에 `management.endpoint.health.group.liveness.include=livenessState,kafka` 로 Kafka indicator include 하여 "Kafka 끊김 검출" 을 목표했으나, **Spring Boot 3.x stock 에 `KafkaHealthIndicator` 가 존재하지 않음**(Boot 2.x→3.x 로 오면서 stock 에서 제거, `spring-boot-actuator-autoconfigure-3.5.0` 의 `AutoConfiguration.imports` 에 Kafka health auto-config 부재. spring-kafka-3.3.6 에도 없음. 프로젝트 커스텀 `HealthIndicator` 빈 0건). `IncludeExcludeGroupMemberPredicate` 가 미매칭 토큰을 silent 무시하므로 dead config 였다. 코드 리뷰 발견 후 worker liveness 그룹 정의 자체를 제거하고 api 와 동일하게 기본 그룹(`livenessState`) 만 사용 — **정직한 후퇴**. PR3 의 가치는 "actuator HTTP probe 도입 + CD 폴링 격상 + JVM 부팅 완료/Spring self-down/9091 응답 가능 검출" 로 명확히 축소. 컴포넌트 사고(Kafka 연결 끊김 / KafkaListenerContainer.stop() / Redisson 락 hold / 스케줄러 풀 굶음) 의 검출은 P1-H Prometheus alert 결합 또는 커스텀 `KafkaHealthIndicator` 빈 신설 후속 트랙으로.
  - **CD 배포 중 false positive 비용**: healthcheck 첫 healthy 도달 윈도우(30~90s)에 일시 fluctuation(Kafka health 미사용이라 현 PR3 에선 사실상 0이지만, 향후 indicator 추가 시) 이 겹치면 CD 폴링 차단 → 사람 개입 비용. 같은 호스트 + `depends_on: service_healthy` 게이트로 빈도 낮으나 운영 매뉴얼에 반영 권장.
