...
> 작업 진입 시 가장 먼저 채우는 정제 섹션. 여기가 비면 §1~§8 전체가 추측에 기반한다.
> 사용자가 의뢰 시 일부를 제공하면 그대로 옮기고, 빈 칸은 "확인 필요"로 표기 — 추측 금지.

### 관련 문서
> 결정 근거가 되는 ADR · spec · research · migration · biz-logic 경로.
> 코드 파일(재사용·수정·참조 대상)은 여기가 아니라 §5 Step의 "읽어야 할 파일"에 적는다.
- <TODO: 예) `docs/architecture/adr/ADR-019-...md`, `docs/research/...`. 없으면 "없음">

### 최신 Source of Truth
> 같은 주제에 여러 문서·코드가 있을 때 **무엇이 결정본인지** 명시.
> ADR-005~023 · research · migration · biz-logic 이 누적되어 충돌 가능성이 있음.
- <TODO: 예) "검색 키워드 정책은 ADR-019 가 결정본. ADR-016 §3 은 superseded.">

### 이전 작업에서 가져오면 안 되는 가정
> `start-task.sh --with-context` 트레젝토리 주입 또는 유사 PR 패턴을 무의식적으로
> 차용할 위험을 차단한다. 채울 게 없어도 생략하지 말고 "없음"으로 명시.
- <TODO: 예) "PR #79 의 LIKE 검색 패턴을 가져오지 말 것 — 이번엔 search_text 합성 컬럼 사용">
...
**읽어야 할 파일** (작업 전 파악 — 코드 파일만. 결정 근거 문서는 §0 에 적는다):
...
0. **컨텍스트 우선**: §0 을 먼저 채운다. 관련 문서·SoT·금지 가정이 비면 §1~§8 이 추측이 된다.
