Feature: 카탈로그 아이템 일괄 등록
  인증된 사용자는 카탈로그 아이템을 일괄로 등록할 수 있으며, 각 항목은 독립된 트랜잭션으로 처리된다.

  @catalog @bulk @smoke
  Scenario: 축구화 1건 + 유니폼 1건을 일괄 등록하면 created=2 가 반환된다
    Given 카카오 인가 코드 "valid-code-bulk-smoke"로 가입한 사용자가 존재한다
  ...
  Scenario: 풋살화 (BOOTS + StudType.TF) 1건을 일괄 등록한다
    Given 카카오 인가 코드 "valid-code-bulk-futsal"로 가입한 사용자가 존재한다
  ...
  Scenario: items 가 빈 배열이면 400 INVALID_INPUT 이 반환된다
    Given 카카오 인가 코드 "valid-code-bulk-empty"로 가입한 사용자가 존재한다
