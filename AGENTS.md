...
이 파일은 Codex가 GearShow 저장소에서 작업할 때 따르는 최상위 지침이다.
Claude Code용 원본 지침은 `CLAUDE.md`와 `.claude/**`에 남겨 두며, Codex는 아래 매핑에 따라 같은 정책을 적용한다.
...
**트리거:** 소스 코드 변경이 예상되는 작업(구현·수정·리팩토링·버그) 요청 시 반드시 `.claude/skills/orchestrator/SKILL.md`를 작업 규칙으로 읽고 따른다. 단순 질문·코드 설명·로그 분석은 직접 응답 가능.

**진입점:** `bash scripts/start-task.sh <task-name> <type>` — worktree/플랜/포트/로그를 원자적으로 생성.

**Codex 적용 방식:**
- Codex는 Claude Code의 `.claude/settings.json` 훅을 자동 실행하지 않는다.
- 대신 이 `AGENTS.md`를 기준으로 동일한 절차를 수동으로 강제한다.
- 소스 코드 변경 전에는 반드시 worktree와 EXEC_PLAN 상태를 확인한다.
- 작업 종료 전에는 계산적 검증 명령을 직접 실행하고 실패 시 자가수정한다.
- 훅 스크립트는 정책의 실행 가능한 명세로 취급한다. 필요하면 내용을 읽어 같은 판단을 적용한다.

**Claude Code 강제 메커니즘 (참고):**
- `PreToolUse` (Edit/Write): `tools/hooks/enforce-worktree.sh`, `tools/hooks/enforce-plan.sh`
- `PreToolUse` (Bash): `tools/hooks/guard-bash.sh`
- `UserPromptSubmit`: `tools/hooks/suggest-worktree.sh`
- `Stop`: `tools/hooks/verify-and-block.sh` (자가수정 루프, 최대 3회)

**Codex 수동 체크리스트:**
1. 구현성 요청이면 관련 문서와 `.claude/skills/orchestrator/SKILL.md`를 먼저 읽는다.
2. 현재 위치가 메인 작업 디렉토리의 `main`/`master`이고 코드 변경이 필요하면 `bash scripts/start-task.sh <task-name> <type>`로 linked worktree를 만든 뒤 그 worktree에서 작업한다.
3. `EXEC_PLAN.md`가 없거나 `<TODO:...>`가 남아 있으면 코드 파일(`.java`, `.gradle`, `.yml`, `.properties`, `.sql`)을 수정하지 않는다.
4. Bash 실행 전 `tools/hooks/guard-bash.sh`의 차단 패턴에 해당하는 위험 명령은 실행하지 않는다.
5. 종료 전 worktree에서 `cd backend && ./gradlew compileJava archTest`를 최소 검증으로 실행한다. 작업 범위가 넓으면 `./gradlew test` 또는 `./gradlew build`까지 실행한다.
6. 검증 실패 시 원인을 수정하고 최대 3회까지 재검증한다. 3회 연속 실패하면 `blocked` 상태로 보고한다.
...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-05-13 | Codex용 AGENTS.md 분리 및 수동 훅 매핑 추가 | AGENTS.md | Claude Code 전용 훅/스킬 전제를 Codex 작업 규칙으로 변환 |
